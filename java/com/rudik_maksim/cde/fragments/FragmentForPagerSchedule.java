 ...
 */
public class FragmentForPagerSchedule extends Fragment {
    ...
    public static FragmentForPagerSchedule newInstance(int position) {
        FragmentForPagerSchedule f = new FragmentForPagerSchedule();
        Bundle b = new Bundle();
    ...
    public void onResume()
    {
        ...
        if (!Global.DataLoaded.Schedule){
            if (!Global.Loading.Schedule){
                Global.Loading.Schedule = true;
                new AsyncGetSchedule().execute();
            }
        }
        else{
            if (!Global.Loading.Schedule){
    ...
    class AsyncGetSchedule extends AsyncTask<Void,Void,Void> {
        ...
        protected void onPostExecute(Void result){
            ...
            Global.DataLoaded.Schedule = true;
    ...
    public void prepareSchedule(boolean isEvenWeek){
        int len = 0;
        try{
            len = Global.CDEData.S_DATA.get(0).size();
        }catch (Exception ex){}
        ...
        Global.CDEData.S_DATA.get(0).toArray(day_week);
        Global.CDEData.S_DATA.get(1).toArray(week_type);
        Global.CDEData.S_DATA.get(2).toArray(time);
        Global.CDEData.S_DATA.get(3).toArray(room);
        Global.CDEData.S_DATA.get(4).toArray(place);
        Global.CDEData.S_DATA.get(5).toArray(title_subject);
        Global.CDEData.S_DATA.get(6).toArray(person_title);
        Global.CDEData.S_DATA.get(7).toArray(status);
